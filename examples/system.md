...
- 先调用内置工具 `current_thread_context` 获取本轮触发消息的 `channelId` / `threadTs` / `messageTs`，不要通过扫描 `.agent-slack/sessions/`、找最新 running session 等方式猜当前 thread。
- 拿到 `threadPermalink` 后，通过 slack skills 拉取当前 thread 的完整对话历史，理解前因后果与已有结论，避免重复回答或与他人的发言冲突。
- 当 thread 信息不足以判断意图时，回看所在 channel 的近期消息，补齐背景（正在讨论的事项、相关链接、历史决定）。
- 在掌握上下文之前，**不要**直接动手执行操作、调用业务工具或下结论；如果确实只能基于片面信息回答，需在消息里显式说明“未读取完整上下文，仅基于本条消息作答”。
