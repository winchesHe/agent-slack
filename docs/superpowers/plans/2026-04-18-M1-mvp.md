...
- [ ] **Step 1: 写 tsdown.config.ts（M4 才真正用）**
...
- [ ] **Step 1: 写 redactor.test.ts（失败测试）**
...
- [ ] **Step 1: 写测试（加载 + 默认值 + system.md 缺失）**
...
- [ ] **Step 1: 写测试（覆盖 getOrCreate 新建 + 重复加载 + append + accumulateUsage）**
...
- [ ] **Step 2: 写 agent/AgentExecutor.ts**
...
- [ ] **Step 1: 写测试（使用 AI SDK 的 `MockLanguageModelV1`）**
...
- [ ] **Step 1: 写测试（mock executor yield 3 个事件）**
...
- [ ] **Step 2: 实现 ConversationOrchestrator**
...
- [ ] **Step 2: 实现 createApplication.ts**
...
- [ ] **Step 4: 在 Slack channel 里 `@your-bot 你好`**
...
- [ ] **Step 5: 在同 thread 再发一条**

验证上下文延续（第二次的 `messages.jsonl` 应在原文件追加，共 4 行）。

- [ ] **Step 6: 验收后 Commit M1 完成标记**
...
- [ ] `pnpm install && pnpm lint && pnpm test && pnpm typecheck` 全绿
- [ ] `pnpm smoke` 能跑通（litellm 可达时）
- [ ] `pnpm dev` 能启动并连接 Slack socket
- [ ] 真 Slack `@mention` 能回复
- [ ] session 目录 + `messages.jsonl` + `meta.json.usage` 正确持久化
- [ ] 日志文件 `logs/YYYY-MM-DD.log` 有输出且凭证被脱敏
- [ ] Task 3.6 手动验收全部 Expected 通过
