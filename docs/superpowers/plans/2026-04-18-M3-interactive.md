...
Expected：回复用正式书面语；`logs/...` 中 system prompt debug 日志包含 "Available Skills"。
...
- A 立即处理
- B、C 原消息出现 ⏳ reaction
- 一条处理完下一条开始，jsonl 顺序 A-user → A-assistant → B-user → ...
- 最终三条都有 ✅
...
- 流式输出停止
- 占位消息变 `⚠️ 用户已停止`，原消息 ❌
- `meta.json.status = 'idle'`（不是 error）
- jsonl 追加 `{ role: 'assistant', content: '[stopped]' }`
...
- [ ] tool-call + tool-result 完整持久化到 jsonl
- [ ] 重新加载历史后 agent 能引用上一轮 tool 输出
