...
**日志格式**（multi 模式增加 `agent=` 字段，single 模式 `agent=default`）：
```
2026-04-27T12:34:56.789Z  INFO [agent-slack] agent=pm taskId=tsk_01HZ... sessionKey=C0AAA.thread.1714.../pm model=claude-opus-4-7 turn=3 tokens.in=1200 tokens.out=240
```
...
├── worktrees/                   # ★ 新增：per-task git worktree 根目录
│   └── <task_id>/               # Coding 在此目录下工作（git worktree add 出来）
...
interface A2AEnvelope {
  ...
  to: 'pm' | 'coding' | 'cs' | 'thread'  // 'thread' = 出口 envelope，
                                         //   见下方语义说明
...
**`to` 字段语义**：
- `to ∈ {'pm','coding','cs'}`：A2A 内部消息，**只落盘 + 投递到目标 Agent 的内存队列，不发 Slack**
- `to === 'thread'`：出口 envelope，**同时落盘 + 调用 SlackAdapter 把 content 发到原 thread**。
  仅 PM 写出口 envelope（进度短消息 / 收尾），Coding / CS 不直接写 thread。

**`references[].value` 格式约定**：
- `kind: 'file'` → 工作区相对路径或绝对路径，可带 `:line` 后缀，如 `src/foo.ts:42`
- `kind: 'url'` → http(s) URL
- `kind: 'session'` → `<channel>.<channelId>.<threadTs>/<agentId>`
- `kind: 'envelope'` → 另一个 envelope id（`env_...`）
...
update_task_board(facts?: string[], decisions?: string[], openQuestions?: string[])
  → { ok: true }                            // 见 §5.3
```

`delegate_to` 时序（关键，决定 P0 实现）：

```
[Agent A turn N，模型调用 delegate_to('B', ...)]
  ├─ tool 执行：
  │   1. 校验 agent !== self
  │   2. 写 envelope 文件 (intent='delegate', from='A', to='B')
  │   3. 投递到内存 A2A bus → 进入 B 的 inbox
  │   4. tool 同步返回 { envelopeId, status: 'queued' }
  │
  ├─ Agent A 的本 turn 继续按模型输出收尾：
  │   - 模型若继续输出 finish_reason='tool_calls' 之外的内容则正常结束 turn
  │   - 模型可在同一 turn 输出 `<waiting/>` 标记（约定信号）告诉 runtime
  │     "我在等 B 回复"
  │
  ├─ runtime 检测 turn 结束 + Agent A 的最后输出含 `<waiting/>`：
  │   - 把 Agent A 的 SessionRunQueue 标记 paused（不再消费新 InboundMessage）
  │   - 不立即开始 A 的 turn N+1
  │
  ├─ B 被 A2A bus 唤醒 → B 跑 turn 处理 delegate
  │   B 完成后通过模型再次调用 delegate_to('A', ..., parentId=<envA>) intent='reply'
  │
  └─ A 的 inbox 收到 reply envelope：
      - runtime 把 reply 内容作为新的 InboundMessage 注入 A 的 ConversationOrchestrator
      - A 的 SessionRunQueue 解 paused，开始 turn N+1，模型看到 reply 后继续
```

要点：
- **`delegate_to` tool 调用本身是同步返回 `queued` 的**，不阻塞 step
- **turn 边界是分开的**：A 的 turn N 结束 → B 跑若干 turn → A 的 turn N+1 开始
- **唤醒机制**：reply envelope 被注入为 A 的下一条 inbound message，由现有 SessionRunQueue 调度
- **`<waiting/>` 标记**只在模型输出层面起约定作用；如果模型未输出该标记，runtime 仍按"模型这一 turn 没产生新 user-visible 输出"判定继续等待

`escalate_to_user` 行为：
1. 写 envelope（`to: 'thread'`，标记为 escalation）
2. 通过 SlackAdapter 在 thread 里 @ 原始用户发问
3. `task.state → 'awaiting_user'`
4. 用户在 thread 内回复后，runtime 把回复打包成 `from: 'user', to: 'pm'` 的 envelope 投递（同上唤醒流程）
...
**写规约**：
- **首次创建**：MentionCommandRouter 在判定一条 mention 创建新 task 时写出 `task.json` 初始版本（goal 暂为空字符串，state='active'，scratchpad 全空）。PM 在第一次 turn 处理用户原文后调用 `update_task_board` 写 goal 摘要。
- **每次 turn 开头**：runtime 读 `task.json` 注入到 system prompt 上下文（结构化转 markdown，见下方 prompt 渲染规约）
- **每次 turn 结尾**：Agent 通过 `update_task_board` tool 追加 facts / decisions / openQuestions（追加，不覆盖；同字符串去重）
- **文件锁**：写时 `task.json.lock`（系统级 advisory lock，retry 5 次×100ms）；读不加锁（最终一致足够）

**Prompt 渲染规约**（runtime 注入 system prompt 之前）：

```
## Task Board (task_id=<id>, state=<state>)
Goal: <goal 或 "(待 PM 设定)">
...
- <question 1>
...
（无 openQuestions 时省略整段）
```

并发写策略 v1：因 v1 严格串行（同一 task 同一时刻仅一个 active agent；SubAgent 共享父 Agent 的 update_task_board 调用，不独立写），实际并发写极少；文件锁仅作为防御性保险。

### 5.4 Loop 终止

- PM 输出 envelope `intent: 'final'` 且 `to: 'thread'` 时，task state → `done`
- 没有 hop 上限 / 时长上限（用户决策）
- **用户主动 abort**（`stop` 命令 / 反应停止 emoji）：
  - **作用域 = 整个 task**：不区分 agent，task 内所有正在跑的 turn 全部中止
  - 行为：(a) `task.state → 'aborted'` (b) 通过 AbortRegistry 向 task 内所有 agent 的 inflight turn 发送 abort 信号 (c) 清空 A2A bus 中该 task 的待投 envelope (d) PM 在 thread 发一条简短"已中止"消息
  - 来源识别：当前 thread 上下文 → 找到 task → 取所有 agent 的 sessionKey 集合做 abort
...
### 6.4 system prompt 拼装规约

`RolePromptLoader` 接口：

```ts
interface RolePromptLoader {
  loadSystemPrompt(workspaceRoot: string, role: 'generic' | 'pm' | 'coding' | 'cs'): string
}
```

拼装策略：
- `role === 'generic'`（单 Agent 模式）：只读 `system.md`，原样返回
- `role ∈ {'pm','coding','cs'}`（multi-agent 模式）：读 `system.md` + 读 `system.<role>.md`，按以下格式拼接：
...
- `system.md` 缺失时仅返回 role overlay；`system.<role>.md` 缺失时退化为只用 base + 在 logs 里 warn 一次（不阻塞启动）
- 拼装在 application 启动时一次性完成；运行期不重读（与现有 `loadWorkspaceContext` 行为一致）

`RolePromptLoader` 是纯函数 + 文件 IO，无状态，便于单元测试。

### 6.5 worktree 物理位置

`WorktreeManager` 在 `<workspaceRoot>/.agent-slack/worktrees/<task_id>/` 下创建 git worktree（`git worktree add <path> <branch>`）：
- 同 task 复用同一 worktree（PM/Coding 多 turn 共享）
- worktree 的 git branch：`agent-slack/task/<task_id>`，从当前 HEAD 切出
- task `state→done|aborted` 后异步标记可清理（保留 7 天再 `git worktree remove`，防止用户回看）
- 清理调度：daemon 启动时扫一次 + 每天凌晨扫一次（用 setInterval）
...
**反问期间允许链式 delegate**（例如 Coding 反问"模块 Y 的 owner 是谁" → PM 可以
delegate CS 查 jira / slack 历史拿到答案再回 Coding），但同一个 question 不要在
PM/Coding/CS 之间循环超过 2 次往返；超过则 PM 直接基于已知信息拍板或 escalate。
...
**`--to=single` 步骤**（multi → single）：
1. 校验当前是 multi（agents 长度 > 1）
2. 备份 config.yaml
3. 默认保留 PM 配置作为 `id='default', role='generic'`（用户可用 `--keep=<agentId>` 指定保留谁）
4. 重写 config.yaml：agents 缩回 1 项；slack env 字段改回无后缀的 `SLACK_BOT_TOKEN`/`SLACK_APP_TOKEN`/`SLACK_SIGNING_SECRET`
5. **历史 sessions 保留原状不动**（`sessions/slack/<thread>/{pm,coding,cs}/` 仍在磁盘上作为存档），新会话写入 `sessions/slack/<thread>/default/`。`--purge` 时连同 `tasks/`、`worktrees/`、`system.<role>.md` 一并删除
6. 打印下一步：清理多余 SLACK_*_<role> env → `agent-slack doctor` → `agent-slack start`
...
| **跨 agent 决策** | `@PM 修复 repo 里那个 bug` → PM delegate Coding → Coding `delegate_to('pm','改这个会影响 module Y, 是否继续')` → PM 自主决策回复 → Coding 完成 patch | (a) envelope 序列含 pm→coding / coding→pm / pm→coding / coding→pm；(b) PM 被反问到回复的端到端 e2e 时延 < 30s（**测试断言，非 runtime 强制**）；(c) Coding 工作目录在独立 worktree |
...
- 新增 `/api/tasks`：列出最近 N 个 task（含 active/done/aborted）
  - 默认查询参数：`?days=30&limit=100&state=*`，可改 `days` 调整时间窗口
- 新增 `/api/tasks/:id`：返回 task.json + envelope 列表
  - 当 task 内 envelope 总数 > 50 时附加 `warnings: ['envelope_count_high']`（见 §14.7）
...
| # | 子项目 | 单位价值 | 关键产出 |
|---|---|---|---|
| **P0** | **Multi-Agent Runtime** | 两个 Agent（PM + Coding）在**纯 fixture 环境**（不接 Slack）能跑通一次完整 A2A 来回 + worktree | A2ABus / TaskBoard / WorktreeManager / RolePromptLoader / `delegate_to` / `escalate_to_user` / `update_task_board` tool / config 自动迁移 / ConversationOrchestrator 重构（单 Agent 回归通过）。**P0 不接 Slack**（Slack 路由在 P1）；P0 的验收靠单元测试 + 集成测试直接投 envelope / mock InboundMessage 完成 |
...
4. **task.json 并发写**：v1 严格串行（同一 task 同一时刻仅一个 active agent；SubAgent 共享父 Agent 的 update_task_board 调用，不独立写），实际并发写不会发生。文件锁仅作为防御性保险（见 §5.3）。
...
7. **PM 死循环**：用户决策不限 hop。极端情况 PM 自我反问可能死循环。**靠 system prompt 自律**（明确写"不要自我反问，直接决策"）+ §7.1 的"同一 question 链式 delegate ≤ 2 次往返"软规约 + dashboard 在 task 内 envelope 数超过阈值（默认 50）时打 warn 标记（不阻断，仅运维可见）。不加运行期硬限。
