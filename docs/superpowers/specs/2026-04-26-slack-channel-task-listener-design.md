...
| bot 以 bot user 身份发普通消息，存在 `user` | `user` | `event.user` | `source.userIds` |
...
- 新增 `channel-task-bot-message`（可选）：若测试环境有可控 bot 消息来源，验证 `bot_message` + `botIds/appIds` 匹配。
